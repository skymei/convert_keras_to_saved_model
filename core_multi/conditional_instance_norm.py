...
class ConditionalInstanceNormalization(Layer):
    ...
    def build(self, input_shape):
        ...
        if self.sw is None:
            # sw is none means using non-conditional instance form
            if self.scale:
            ...
            sw_shape = K.int_shape(self.sw)
            new_shape = (sw_shape[-1], shape[0])
            self.beta = self.add_weight(shape=new_shape,
                                        name='beta',
                                        initializer=self.beta_initializer,
                                        regularizer=self.beta_regularizer,
                                        constraint=self.beta_constraint,
            ...
            self.gamma = self.add_weight(shape=new_shape,
                                         name='gamma',
                                         initializer=self.gamma_initializer,
                                         regularizer=self.gamma_regularizer,
                                         constraint=self.gamma_constraint,
